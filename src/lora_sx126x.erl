...
waiting_tx_done(info, {gpio_interrupt, Pin}, #state{irq = Pin} = State) ->
    ?TRACE("gpio_interrupt IRQ Pin=~p Level=~p in waiting_tx_done state~n", [Pin, gpio:digital_read(Pin)]),
...
waiting_tx_done(EventType, Request, State) ->
    ?TRACE("Unhandled message in waiting_tx_done state.  EventType: ~p Request: ~p~n", [EventType, Request]),
